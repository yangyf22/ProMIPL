...
class MIPLDataloader(data_utils.Dataset):
    def __init__(self, all_ins_fea, bag_idx_of_ins, dummy_ins_lab, bag_lab, partial_bag_lab_processed, idx_list,
                 nr_fea, ds_name, seed=1, train=True, normalize=False):
        ...
        self.bags_list, self.ins_lab_list, self.partial_bag_lab_list, \
        self.true_bag_lab_list = self._create_bags()
    ...
    def __len__(self):
        return len(self.ins_lab_list)

    def __getitem__(self, index):
        bag = self.bags_list[index]
        partial_bag_label = self.partial_bag_lab_list[index]
        true_bag_label = self.true_bag_lab_list[index]
